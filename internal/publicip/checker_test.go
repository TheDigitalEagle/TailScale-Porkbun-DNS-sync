...
func TestCheckerIPv6(t *testing.T) {
	t.Parallel()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte("2001:db8::8\n"))
	}))
	defer server.Close()

	checker := NewChecker(server.URL)
	addr, err := checker.IPv6(context.Background())
	if err != nil {
		t.Fatalf("IPv6() error = %v", err)
	}

	if got, want := addr.String(), "2001:db8::8"; got != want {
		t.Fatalf("IPv6() = %s, want %s", got, want)
	}
}

func TestCheckerIPv6RejectsNonIPv6(t *testing.T) {
	t.Parallel()
	...
	checker := NewChecker(server.URL)
	if _, err := checker.IPv6(context.Background()); err == nil {
		t.Fatal("IPv6() error = nil, want non-nil")
	}
}
