...
func TestHealthEndpoint(t *testing.T) {
	...
	server := NewServer(Config{Domain: "ima.fish"}, &fakeRunner{}, fakeLister{})
...
func TestSyncRunEndpoint(t *testing.T) {
	...
	server := NewServer(Config{Domain: "ima.fish"}, runner, fakeLister{})
...
func TestPublicRecordsEndpoint(t *testing.T) {
	...
	server := NewServer(Config{Domain: "ima.fish"}, &fakeRunner{}, fakeLister{
		records: []porkbun.Record{
			...
			{Name: "ima.fish", Type: "A", Content: "198.51.100.11", TTL: "600"},
		},
	})
...
func TestGroupPublicRecords(t *testing.T) {
	t.Parallel()

	records := groupPublicRecords([]porkbun.Record{
		{Name: "www", Type: "a", Content: netip.MustParseAddr("198.51.100.20").String(), TTL: "600"},
		{Name: "www.ima.fish", Type: "A", Content: netip.MustParseAddr("198.51.100.21").String(), TTL: "600"},
	}, "ima.fish")

	if got, want := len(records), 1; got != want {
		t.Fatalf("len(records) = %d, want %d", got, want)
	}
	if got, want := records[0].FQDN, "www.ima.fish"; got != want {
		t.Fatalf("FQDN = %q, want %q", got, want)
	}
	if got, want := len(records[0].Values), 2; got != want {
